...
edu774$is_primary <- edu774$level_of_education %in%  c('primary_only', 'preprimary_and_primary',
                                             'primary_and_junior_sec', 'primary_junior_and_senior_sec',
                                             'primary', 'preprimary', 'primary_js',
                                             'primary_js_ss', 'preprimary_primary')


edu774$is_junior_secondary <- edu774$level_of_education %in% c('juniors_sec_only', 
                                                               'junior_and_senior_sec', 
                                                               'js', 'js_ss')

edu774$pj <- edu774$is_primary | edu774$is_junior_secondary
...
lga_edu_data <- ddply(ie, .(lga_id), function(df) {
   data.frame(
              ...
              num_senior_secondary_schools = icount(df$level_of_education %in% c('senior_sec_only','ss')),
...
#writing out
# saveRDS(x_y_killa(lga_education_all), "~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/in_process_data/Normalized/normalized_final/Education_LGA_level_774.rds")
write.csv(x_y_killa(lga_education_all), "~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/in_process_data/Normalized/normalized_final/Education_LGA_level_774.csv", row.names=F)
