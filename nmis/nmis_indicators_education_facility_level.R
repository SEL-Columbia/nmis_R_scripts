...
e <- readRDS("~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/in_process_data/outlier_cleaned/Education_661_outliercleaned.rds")
lga_661 <- read.csv("~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/661.csv",stringsAsFactors=F)
                                         ...
                                         ifelse(e$school_managed == "priv_profit", "private",      
                                                ifelse(e$school_managed == "priv_noprofit", "private",      
                                                       ifelse(e$school_managed == "faith_org", "private",      
                                                              NA_character_))))))
ed$num_tchrs_total <- e$num_tchrs_total
ed$num_students_total <- e$num_students_total
...
ed$num_tchrs_with_nce <- e$num_tchrs_w_nce

ed$num_textbooks <-  
  ifelse(e$level_of_education %in% c('primary_only', 'preprimary_and_primary'),  
     e$num_math_textbook_pry + e$num_english_textbook_pry + 
           e$num_soc_science_textbook_pry + e$num_science_textbook_pry,
  ...
  ifelse(e$level_of_education %in% c('primary_and_junior_sec', 'primary_junior_and_senior_sec'),
    e$num_math_textbook_pry + e$num_english_textbook_pry + 
      ...
      e$manuals_js.num_math_textbook_js + e$manuals_js.num_english_textbook_js + 
           e$manuals_js.num_soc_science_textbook_js + e$manuals_js.num_science_textbook_js,
  0)))
ed$textbook_to_pupil_ratio <- ed$num_textbooks / e$num_students_total
...
## PARTICIPATION ##
ed$male_to_female_student_ratio <- e$num_students_male / 
  e$num_students_female
...
ed$gender_separated_toilets_yn <- (e$num_toilet_boy > 1) & (e$num_toilet_girl > 1)
ed$pupil_toilet_ratio_facility <- e$num_students_total / 
  (e$num_toilet_boy + e$num_toilet_girl + e$num_toilet_both)
...
ed$pupil_classrm_ratio <- e$num_students_total / e$num_classrms_total
...
## Furniture ##
ed$pupil_bench_ratio <- e$num_students_total / e$num_benches
ed$pupil_desk_ratio <- e$num_students_total / e$num_desks

## Adequacy of Staffing ##
ed$pupil_tchr_ratio <- e$num_students_total / e$num_tchrs_total
ed$teacher_nonteachingstaff_ratio <- e$num_tchrs_total / (e$num_sr_staff_total)
ed$num_tchrs_with_nce <- e$num_tchrs_w_nce
