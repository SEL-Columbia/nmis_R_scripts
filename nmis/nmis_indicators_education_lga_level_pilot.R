...
ed <- readRDS("~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/in_process_data/nmis/data_pilot/Education_Pilot_ALL_FACILITY_INDICATORS.rds")
e <- ed

e$is_primary <- e$level_of_education %in% 
  c('primary', 'preprimary_primary', 'primary_js',
    'primary_js_ss')
e$is_junior_secondary <- e$level_of_education %in% 
  c('js', 'js_ss')
e$pj <- e$is_primary | e$is_junior_secondary
e$num_classrms_total <- rowSums(e[,c("num_classrms_need_maj_repairs", 
                                     "num_classrms_need_min_repairs", 
                                     "num_classrms_good_cond")], na.rm=T)
e$num_students_total_gender.num_students_total <- rowSums(e[, c("num_students_female",
...
lga_edu_data <- ddply(ie, .(lga_id), function(df) {
  data.frame(
    ...
    #         benches_school_ratio = ratio(df$num_benches, df$pj), #num_benches NOT EXIST
    ...
    #         classrooms_num_chalkboards = sum(df$num_classrm_w_chalkboard, na.rm = TRUE), #num_classrm_w_chalkboard NOT Exist
    ...
    percent_teaching_guides = bool_proportion(df$teacher_guide_yn, T),
    ...
    #             bool_proportion(df$km_to_catchment_area > 1, df$is_primary), #km_to_catchment_are NOT EXIST
    ...
    #             bool_proportion(df$wall_fence_good_condi, df$is_primary), #roof_fence_good_condition & wall_fence_good_condi NOT EXIST
    ...
    #                   df$num_benches, df$is_primary),                  # num_benches NOT EXIST               
    ...
    #             ratio(df$num_students_total_gender.num_students_total, df$num_tchrs.num_tchrs_total, df$is_primary), #num_tchr NOT EXIST
    ...
    #             ratio(df$num_tchrs.num_tchrs_total, df$num_sr_staff_total + df$num_jr_staff_total, df$is_primary), #num_tchr, num_jr_staff, num_sr_staff not available
    ...
    #             ratio(df$num_tchrs_with_nce, df$num_tchrs.num_tchrs_total, df$is_primary), #num_tchr_with_nce & num_tchr_total NOT EXIST
    ...
    #             bool_proportion(df$provide_exercise_books_yn == 'yes', df$is_primary), #provided_exercise_books NOT EXist
    #         proportion_provide_exercise_books_juniorsec =
    #             bool_proportion(df$provide_exercise_books_yn == 'yes', df$is_junior_secondary),
    ...
    num_other_level = icount(df$level_of_education %in%
                               c('science_technical', 'vocational_post_primary')),
...
#writing out
saveRDS(x_y_killa(lga_education_all), "~/Dropbox/Nigeria/Nigeria 661 Baseline Data Cleaning/in_process_data/nmis/data_pilot/Education_LGA_level_pilot.rds")
