...
ed <- read.csv("in_process_data/nmis/data_661/Education_661_ALL_FACILITY_INDICATORS.csv", stringsAsFactor=F)

e <- ed
e$is_primary <- e$level_of_education %in% 
  c('primary_only', 'preprimary_and_primary', 'primary_and_junior_sec',
    'primary_junior_and_senior_sec')
e$is_junior_secondary <- e$level_of_education %in% 
  c('juniors_sec_only', 'junior_and_senior_sec')
...
lga_edu_data <- ddply(ie, .(lga_id), function(df) {
   data.frame(
              ...
              schools_use_teaching_aids = icount(df$teacher_guide_yn),
                ...
                num_senior_secondary_schools = icount(df$level_of_education == 'senior_sec_only'),
        ...
        proportion_schools_functioning_library_juniorsec = 
          bool_proportion(df$functioning_library_yn, df$is_junior_secondary),                            
                num_preprimary_level =
                  icount(df$level_of_education == 'preprimary_only'),              
                num_preprimary_primary_level = 
                  icount(df$level_of_education == 'preprimary_and_primary'),              
                num_primary_level =
                  icount(df$level_of_education == 'primary_only'),              
                num_primary_js_level =  
                  icount(df$level_of_education == 'primary_and_junior_sec'),              
                num_js_level =
                  icount(df$level_of_education == 'juniors_sec_only'),              
                num_js_ss_level = 
                  icount(df$level_of_education == 'junior_and_senior_sec'),              
                num_ss_level = 
                  icount(df$level_of_education == 'senior_sec_only'),            
                num_primary_js_ss_level = 
                  icount(df$level_of_education == 'primary_junior_and_senior_sec'),
...
#writing out
write.csv(lga_education_all, "in_process_data/nmis/data_661/Education_LGA_level_661.csv", row.names=F)
